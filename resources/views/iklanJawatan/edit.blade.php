    ...
    <div class="card-body bg-light">
      <div class="tab-content">
        <div class="tab-pane preview-tab-pane active" role="tabpanel" aria-labelledby="tab-dom-bb654daf-be1a-40fe-b687-3e45244388c2" id="dom-bb654daf-be1a-40fe-b687-3e45244388c2">
            <form action="/iklanJawatan/{{ $iklan_jawatan->id }}" method="POST">
                @csrf
                @method('PUT')
                {{-- {{ dd($iklan_jawatan); }} --}}
            ...
            <div class="mb-3">
              ...
              <input class="form-control" name="tarikhMula" type="date" value="{{ $iklan_jawatan->tarikhMula }}" />
            </div>

            <div class="mb-3">
                <label class="form-label" for="tarikhTutup">Tarikh Tutup</label>
                <input class="form-control" name="tarikhTutup" type="date" value="{{ $iklan_jawatan->tarikhTutup }}"/>
              </div>

              <div class="mb-3">
                <label class="form-label" for="jawatan">Jawatan</label>
                <select class="form-select" name="jawatan" type="text">
                  <option selected="selected">{{ $iklan_jawatan->jawatan }}</option>
            ...
            <div class="mb-3">
                ...
                <input class="form-control" name="penempatan" type="text" value="{{ $iklan_jawatan->penempatan }}"/>
            </div>

            <div class="mb-3">
                <label class="form-label" for="kekosongan">Kekosongan</label>
                <input class="form-control" name="kekosongan" type="text" placeholder="" value="{{ $iklan_jawatan->kekosongan }}" />
            </div>

            <div class="mb-3">
                <label class="form-label" for="tarafJawatan">Taraf Jawatan</label>
                <select class="form-select" name="tarafJawatan" type="text">
                  <option selected="selected">{{ $iklan_jawatan->tarafJawatan }}</option>
